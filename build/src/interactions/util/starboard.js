...
const Ping = new Interaction({
    ...
    exec: ({ interaction }) => {
        interaction.reply(`Created a skullboard in ${interaction.options.get("channel").channel} that triggers when ${interaction.options.get("skull_count").value} skull(s) are added.`);
